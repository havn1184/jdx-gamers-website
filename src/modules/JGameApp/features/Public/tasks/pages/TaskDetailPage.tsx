...
import {
  Loader2, AlertCircle, Coins, Users, Clock3, CheckCircle2, PartyPopper, RefreshCw, ListChecks, KeyRound,
} from 'lucide-react'
...
export function TaskDetailPage() {
  ...
  const canRegister = isAuthenticated && !isRegistered && remaining > 0
    ...
    <div className='mx-auto max-w-5xl px-4 py-8 sm:px-6'>
      <div className='grid grid-cols-1 gap-8 lg:grid-cols-2'>
        ...
        <div>
          ...
          {task.description && <p className='mt-2 text-sm text-white/60'>{task.description}</p>}
          ...
          <div className='mt-5 rounded-xl border border-white/10 bg-white/5 p-4'>
            <div className='flex items-center justify-between'>
              ...
              <span className='flex items-center gap-1 font-bold text-amber-300'><Coins className='h-4 w-4' /> {formatNumber(task.rewardJcoin)} JCoin</span>
            </div>
          ...
          {isRegistered && progress ? (
            <div className='mt-5 rounded-xl border border-white/10 bg-white/5 p-4'>
              <div className='flex items-center justify-between'>
                ...
                <Badge className={cn('border-none', progress.isCompleted ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300')}>
                  {progress.isCompleted ? 'Đã nhận thưởng' : 'Đang thực hiện'}
                </Badge>
              </div>
              {progress.registrationCode && (
                <p className='mt-2 flex items-center gap-1 text-xs text-white/50'><KeyRound className='h-3.5 w-3.5' /> Mã đăng ký: <span className='font-mono text-white/80'>{progress.registrationCode}</span></p>
              )}

              <div className='mt-3 flex items-center justify-between text-sm'>
                <span className='text-white/70'>Đã tích lũy</span>
                <span className='font-semibold text-amber-300'>{formatNumber(getEarnedSoFar(task, progress))} / {formatNumber(task.rewardJcoin)} JCoin</span>
              </div>
              ...
              {progress.isCompleted && (
                <div className='mt-3 flex items-center gap-2 rounded-lg bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300'>
                  <PartyPopper className='h-4 w-4 flex-shrink-0' /> Chúc mừng! Bạn đã nhận {formatNumber(task.rewardJcoin)} JCoin vào ví.
                </div>
              )}
            </div>
          ) : (
